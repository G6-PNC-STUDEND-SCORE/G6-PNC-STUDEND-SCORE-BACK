...
class ScoreController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        $query = Score::with([
            ...
            'enrollment.subjectOffering.subject',
            'enrollment.subjectOffering.term',
        ]);

        if ($request->student_id) {
            $query->whereHas('enrollment', function ($enrollmentQuery) use ($request) {
                $enrollmentQuery->where('student_id', $request->student_id);
            });
        }

        if ($request->subject_offering_id) {
            $query->whereHas('enrollment', function ($enrollmentQuery) use ($request) {
                $enrollmentQuery->where('subject_offering_id', $request->subject_offering_id);
            });
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $request->validate([
            ...
            'details.*.type' => "required_with:details|in:{$allowedTypes}",
    ...
    private function assessmentTypeId(string $code): int
    {
        return AssessmentType::firstOrCreate(
            ...
            [
                'name' => ucfirst($code),
                'weight_percent' => AssessmentTypeDefaults::weightFor($code),
                'is_active' => true,
            ]
        )->id;
    }
