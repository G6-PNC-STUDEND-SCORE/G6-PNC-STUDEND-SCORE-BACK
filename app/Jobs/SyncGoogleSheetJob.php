...
class SyncGoogleSheetJob implements ShouldQueue
{
    ...
    public function handle(ScoreCalculationService $scoreService): void
    {
        ...
        try {
            $result = $this->pullFromSheets($link, $scoreService);
    ...
    private function pullFromSheets(GoogleSheetLink $link, ScoreCalculationService $scoreService): array
    {
        ...
        $scoreCols = array_slice($header, 2, count($header) - 4);
        ...
        try {
            foreach (array_slice($values, 1) as $row) {
                ...
                $scoreService->recalculateTotalById($score->id);
            }
            DB::commit();
        } catch (\Throwable $e) {
            DB::rollBack();
